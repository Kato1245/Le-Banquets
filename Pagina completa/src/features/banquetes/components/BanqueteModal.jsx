        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="lg:w-1/2 p-8">
            <header className="mb-6">
              <h2 className="text-3xl font-bold text-primary mb-2">{banquete.nombre}</h2>
              <div className="flex items-center gap-2 text-base-content/70">
                ...
                <span>{banquete.direccion || banquete.ubicacion}</span>
              </div>
            </header>

            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className="bg-base-200 p-3 rounded-xl">
                <p className="text-xs uppercase opacity-50 font-bold mb-1">Precio base</p>
                <p className="text-xl font-bold text-primary">
                  ${banquete.precio_base?.toLocaleString('es-CO') || banquete.precio?.toLocaleString('es-CO')}
                </p>
              </div>
              <div className="bg-base-200 p-3 rounded-xl">
                <p className="text-xs uppercase opacity-50 font-bold mb-1">Capacidad</p>
            ...
            <div className="space-y-4 mb-8">
              <div>
                ...
                <p className="text-sm text-base-content/80 line-clamp-4">
                  {banquete.descripcion || "Un espacio excepcional diseñado para que tu evento sea una experiencia inolvidable. Contamos con todas las facilidades y un ambiente único."}
                </p>
              </div>

              {banquete.servicios && banquete.servicios.length > 0 && (
                <div>
                  <h4 className="font-bold text-sm mb-2">Servicios Incluidos</h4>
                  <div className="flex flex-wrap gap-1">
                    {banquete.servicios.map((s, idx) => (
                      <span key={idx} className="badge badge-sm badge-outline text-xs">{s}</span>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div className="modal-action mt-auto border-t pt-6 border-base-200">
              <button className="btn btn-primary btn-block flex-1" onClick={() => window.location.href = `/banquetes/${banquete._id}`}>
