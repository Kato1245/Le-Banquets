    ...
    return (
        <form onSubmit={handleSubmit} noValidate>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="form-control md:col-span-2">
                    <label className="label">
                        <span className="label-text font-bold opacity-70">Nombre del Banquete <span className="text-error">*</span></span>
                    </label>
                    <input
                        type="text"
                        name="nombre"
                    ...
                    />
                    {errors.nombre && <span className="label-text-alt text-error mt-1 font-medium">{errors.nombre}</span>}
                </div>

                {/* Descripción */}
                <div className="form-control md:col-span-2">
                    <label className="label">
                        <span className="label-text font-bold opacity-70">Descripción <span className="text-error">*</span></span>
                        <span className="label-text-alt opacity-40">{form.descripcion.length} / 20 mín.</span>
                    </label>
                    <textarea
                        name="descripcion"
                    ...
                    />
                    {errors.descripcion && <span className="label-text-alt text-error mt-1 font-medium">{errors.descripcion}</span>}
                </div>

                {/* Ubicación */}
                <div className="form-control">
                    <label className="label">
                        <span className="label-text font-bold opacity-70">Ubicación <span className="text-error">*</span></span>
                    </label>
                    <input
                        type="text"
                        name="ubicacion"
                    ...
                    />
                    {errors.ubicacion && <span className="label-text-alt text-error mt-1 font-medium">{errors.ubicacion}</span>}
                </div>

                {/* Tipo */}
                <div className="form-control">
                    <label className="label">
                        <span className="label-text font-bold opacity-70">Tipo de Espacio</span>
                    </label>
                    <select
                        name="tipo"
                        value={form.tipo}
                ...
                <div className="form-control">
                    <label className="label">
                        <span className="label-text font-bold opacity-70">Capacidad (personas) <span className="text-error">*</span></span>
                    </label>
                    <input
                        type="number"
                        name="capacidad"
                    ...
                    />
                    {errors.capacidad && <span className="label-text-alt text-error mt-1 font-medium">{errors.capacidad}</span>}
                </div>
                ...
                <div className="form-control">
                    ...
                    />
                    {errors.precio_base && <span className="label-text-alt text-error mt-1 font-medium">{errors.precio_base}</span>}
                </div>

                {/* Imágenes */}
                <div className="form-control md:col-span-2">
                    <label className="label">
                        <span className="label-text font-bold opacity-70">Imágenes del espacio <span className="opacity-40 font-normal">(máx. 5)</span></span>
                    </label>
                    <input
                        type="file"
                        accept="image/*"
                    ...
                    {imagePreview.length > 0 && (
                        <div className="flex gap-3 mt-4 flex-wrap">
                            {imagePreview.map((src, i) => (
                                <div key={i} className="relative w-24 h-24 rounded-2xl overflow-hidden border-2 border-primary/30 shadow-md">
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-4 mt-10 pt-8 border-t border-base-content/5">
                <button
                    type="button"
                    onClick={() => { setForm(INITIAL_FORM); setErrors({}); setImagePreview([]); }}
...
const EstadisticasPanel = ({ totalBanquetes }) => {
    const stats = [
        { label: "Banquetes Publicados", value: totalBanquetes, icon: "🏰", color: "text-primary", bg: "bg-primary/10" },
        { label: "Visitas este mes", value: "—", icon: "👀", color: "text-info", bg: "bg-info/10" },
        { label: "Solicitudes recibidas", value: "—", icon: "📩", color: "text-success", bg: "bg-success/10" },
        { label: "Tasa de conversión", value: "—", icon: "📈", color: "text-warning", bg: "bg-warning/10" },
    ];
    ...
    return (
        <div>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                {stats.map((s, i) => (
                    <div key={i} className={`rounded-3xl p-6 border border-base-content/5 ${s.bg} flex flex-col items-center text-center`}>
                        <span className="text-4xl mb-3">{s.icon}</span>
                        <span className={`text-3xl font-black ${s.color} mb-1`}>{s.value}</span>
                        <span className="text-xs font-bold uppercase tracking-widest opacity-50">{s.label}</span>
                    </div>
                ))}
            </div>

            <div className="alert bg-base-200 border border-base-300 rounded-2xl">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 shrink-0 opacity-40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span className="text-sm opacity-60">
                    Las estadísticas detalladas de visitas y solicitudes estarán disponibles próximamente conforme los módulos de seguimiento se activen.
                </span>
            </div>
        </div>
    );
        ...
        <div className="max-w-2xl">
            <div className="card bg-base-200 border border-base-300 rounded-3xl overflow-hidden mb-6">
                <div className="card-body p-8">
                    <div className="flex items-center gap-6 mb-6">
                        ...
                        <div>
                            <h3 className="text-2xl font-black">{user?.nombre || "Propietario"}</h3>
                            <p className="text-sm opacity-60 font-medium">{user?.email}</p>
                            <div className="badge badge-info badge-outline mt-2 font-bold text-[10px] tracking-widest">PROPIETARIO</div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        {[
                            ...
                            { label: "Miembro desde", value: user?.createdAt ? new Date(user.createdAt).toLocaleDateString("es", { year: "numeric", month: "long" }) : "—" },
                            { label: "ID de cuenta", value: user?.id ? `${user.id.substring(0, 12)}...` : "—" },
                        ].map((item, i) => (
                            <div key={i} className="flex justify-between items-center p-4 bg-base-100 rounded-2xl border border-base-content/5">
                                <span className="opacity-50 font-bold uppercase text-[10px] tracking-widest">{item.label}</span>
                                <span className="font-semibold font-mono text-xs">{item.value}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="alert alert-info bg-info/10 border-info/20 rounded-2xl">
                ...
                <span className="text-sm">
                    Para editar los datos de tu perfil, ve a <strong>Configuración de Perfil</strong> desde el menú de usuario.
                </span>
            </div>
        </div>
    );
    ...
    return (
        <div className="card bg-base-100 shadow-xl border border-base-200 overflow-hidden group hover:shadow-2xl transition-all duration-500 rounded-[2rem]">
            <figure className="h-56 relative overflow-hidden">
                <img
                    src={banquete.imagenes?.[0] || "https://images.unsplash.com/photo-1519167758481-83f550bb49b3?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80"}
                    alt={banquete.nombre}
                    className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"
                />
                <div className={`absolute top-4 right-4 badge ${banquete.estado === 'activo' ? 'badge-success' : 'badge-ghost'} py-3 font-bold uppercase text-[10px] tracking-widest shadow`}>
            ...
            <div className="card-body p-7">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-base-content/5">
                    <div className="flex flex-col">
                        <span className="text-[10px] uppercase font-black opacity-30 tracking-tight">Capacidad</span>
        ...
        <div className="min-h-screen bg-base-100 py-12">
            <div className="max-w-7xl mx-auto px-4 md:px-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                    <div>
                        <h1 className="text-4xl font-extrabold tracking-tight mb-1">Panel de Propietario</h1>
                    ...
                    <div>
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                            <div>
                                ...
                                <p className="text-sm opacity-50 font-medium mt-1">
                                    {loading ? "Cargando..." : `${banquetes.length} banquete${banquetes.length !== 1 ? "s" : ""} publicado${banquetes.length !== 1 ? "s" : ""}`}
                                </p>
                            </div>
                            <button
                        ...
                        {loading ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                {[1, 2, 3].map(i => <BanqueteSkeleton key={i} />)}
                            </div>
                        ) : banquetes.length === 0 ? (
                            <EmptyState
                                icon="🏰"
                ...
                {activeTab === "estadisticas" && (
                    <div>
                        <div className="mb-8">
                            <h2 className="text-2xl font-bold mb-1">Estadísticas de tu Portafolio</h2>
                            <p className="text-sm opacity-50 font-medium">Seguimiento del rendimiento de tus espacios publicados.</p>
                        </div>
                ...
                {activeTab === "empresa" && (
                    <div>
                        <div className="mb-8">
                            ...
                            <p className="text-sm opacity-50 font-medium">Información de tu cuenta como propietario verificado.</p>
                        </div>
