...
const MisEventos = () => {
    ...
    try {
      ...
      if (reservasRes.ok) {
        ...
        const formattedReservas = (resData.data || []).map((res) => ({
          ...
          direccion:
            res.banquete_id?.direccion || res.banquete_id?.direccion || "N/A",
        }));
      ...
      if (citasRes.ok) {
        ...
        const formattedCitas = (citaData.data || []).map((cit) => ({
          ...
          direccion:
            cit.banquete_id?.direccion || cit.banquete_id?.direccion || "N/A",
        }));
        consolidatedData = [...consolidatedData, ...formattedCitas];
      }

      setEventos(
        consolidatedData.sort((a, b) => new Date(b.fecha) - new Date(a.fecha)),
      );
    } catch (error) {
  ...
  const eventosFiltrados = eventos.filter((evento) => {
    ...
    if (activeTab === "proximos")
      return fechaEvento >= hoy && evento.estado !== "completado";
    ...
    <div className="min-h-screen bg-base-100 py-16">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div>
            ...
            <p className="text-lg opacity-60 font-medium lowercase italic">
              Supervisión de Reservas y Gestión de Calendario
            </p>
          </div>
        ...
        {eventosFiltrados.length > 0 ? (
          <div className="grid grid-cols-1 gap-8">
            {eventosFiltrados.map((evento) => (
              <div
                key={evento.id}
                className="card bg-base-100 shadow-xl border border-base-200 rounded-[2.5rem] overflow-hidden hover:border-primary/30 transition-all group"
              >
                  ...
                  <div className="flex flex-col lg:flex-row justify-between gap-10">
                    <div className="flex-1">
                      <div className="flex items-center gap-4 mb-4">
                        <div
                          className={`badge ${getBadgeClass(evento.estado)} py-4 px-6 rounded-xl font-black uppercase text-[10px] tracking-[0.2em]`}
                        >
                          {evento.estado}
                        </div>
                      ...
                      <h2 className="text-4xl font-extrabold tracking-tight mb-4 group-hover:text-primary transition-colors">
                        {evento.nombre}
                      </h2>
                    ...
                    <div className="flex flex-col justify-between items-start lg:items-end border-l-0 lg:border-l border-base-content/5 lg:pl-10 min-w-[200px]">
                      ...
                      <div className="bg-base-100/50 p-4 rounded-2xl border border-base-content/5 mb-6 w-full">
                        <div className="flex justify-between items-center mb-1">
                          <p className="text-[10px] uppercase font-black opacity-30">
                            Veredicto
                          </p>
                        ...
                        {evento.motivo_rechazo && (
                          <div className="mt-3 p-3 bg-error/5 rounded-xl border border-error/10 border-l-4 border-l-error">
                            <p className="text-[9px] text-error font-black uppercase mb-1">
                              Explicación del Propietario:
                            </p>
                      ...
                      <div className="flex gap-2 w-full lg:w-auto mt-4">
                        {!isPropietario && evento.category === "reserva" && 
                          (evento.estado === "completado" || 
                          (evento.estado === "confirmada" && new Date(evento.fecha) < new Date())) && (
                          <button
          ...
          <div className="text-center py-32 bg-base-200/30 rounded-[3rem] border border-dashed border-base-content/10">
            ...
            <h3 className="text-2xl font-bold mb-2">
              {isPropietario
                ? "Tu agenda de cliente está vacía"
            ...
            <button
              className="btn btn-primary rounded-xl px-10 shadow-lg"
              onClick={() =>
                navigate(
                  isPropietario
                    ? "/mis-banquetes?tab=calendario"
                    : "/banquetes",
                )
              }
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-16 pt-8 border-t border-base-content/10">
          ...
          <div className="flex flex-col items-center text-center p-8 bg-base-200/50 rounded-3xl border border-base-300">
            <span className="text-4xl font-black text-secondary mb-2 line-clamp-1">
              $
              {(
                eventos.reduce((a, b) => a + (b.precio || 0), 0) / 1000
              ).toFixed(1)}
              k
            </span>
            <span className="text-[10px] uppercase font-bold tracking-widest opacity-40">
          ...
          <div className="bg-base-100 w-full max-w-md rounded-[2.5rem] shadow-2xl border border-base-content/5 overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-8">
              <div className="flex justify-between items-start mb-6">
                <div
                  className={`badge ${getBadgeClass(selectedEvent.estado)} text-white font-black uppercase tracking-tighter text-[10px] py-3 px-4`}
                >
              ...
              <p className="text-xs opacity-50 mb-6 font-bold uppercase tracking-widest">
                Detalle de {selectedEvent.category}
              </p>
                  ...
                  <div>
                    ...
                    <p className="font-bold text-sm">
                      {selectedEvent.precio > 0
                        ? `$${selectedEvent.precio.toLocaleString("es-CO")}`
                        : "Por definir / Gratuita"}
                    </p>
                  </div>
                </div>
